...
g <- function(Yhat, Y, eps = 1e-15) {
  ...
  N <- nrow(Y)
  -sum( Y * log( pmax(Yhat, eps) ) ) / N # pmax() replaces any element of Yhat that is smaller than eps with eps - ensures no value passed to log() is <= zero
}
```
...
af_forward <- function(X, Y, theta, m, v) 
{
  ...
  P_3byN <- softmax(t(Z))   # temporarily transpose because softmax expects input where columns are different samples
  probs   <- t(P_3byN)  
...
The optimal regularization parameter is `r round(optimal_v, 4)` as it minimizes the validation error.
